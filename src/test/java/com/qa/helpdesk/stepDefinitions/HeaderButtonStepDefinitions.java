    ...
    public void verifyIfUserGuideIsOpened() {
        try {
            ...
            if (DriverAction.isExist(DashboardHeaderLocators.pdfHeader) && DriverAction.getElementText(DashboardHeaderLocators.pdfHeader).equalsIgnoreCase("Helpdesk user guide.pdf")) {
                ...
                GemTestReporter.addTestStep("Verify user-guide pdf", "User-guide pdf is not displayed", STATUS.FAIL, DriverAction.takeSnapShot());
    ...
    public void verifyIfSupportInfoIsDisplayed(String info) {
        try {
            if (DriverAction.getElementText(DashboardHeaderLocators.contact).equalsIgnoreCase(info)) {
                ...
                GemTestReporter.addTestStep("Contact info", "Email address is not visible, Expected: " + info + "", STATUS.FAIL, DriverAction.takeSnapShot());
    ...
    public void clickOnNotificationButton() {
        try {
            ...
            if (DriverAction.isExist(DashboardHeaderLocators.notificationButton)) {
                ...
                GemTestReporter.addTestStep("Notification button", "Notification button not visible", STATUS.FAIL, DriverAction.takeSnapShot());
            ...
            if (DriverAction.isExist(DashboardHeaderLocators.notificationContainer)) {
                ...
                if (DriverAction.isExist(DashboardHeaderLocators.showMoreNotifications)) {
                    ...
                    if (DriverAction.getElements(DashboardHeaderLocators.notificationList).size() > initialCount.size()) {
                        ...
                        GemTestReporter.addTestStep("Show more notifications", "More notifications are not displayed", STATUS.FAIL, DriverAction.takeSnapShot());

                } else
                    GemTestReporter.addTestStep("Show more notifications", "Show more button for notifications is not present", STATUS.FAIL, DriverAction.takeSnapShot());
            } else
                GemTestReporter.addTestStep("Notifications", "Notification container not visible", STATUS.FAIL, DriverAction.takeSnapShot());
        } catch (Exception e) {
            GemTestReporter.addTestStep("Exception Occurred", "Exception: " + e, STATUS.FAIL);
            throw new RuntimeException(e);
        }
    ...
    public void checkIfIsThereAnUnreadCountForNotifications() {
        try {
            if (DriverAction.isExist(DashboardHeaderLocators.unreadCount)) {
                ...
                GemTestReporter.addTestStep("Notifications", "No unread count for notifications", STATUS.INFO, DriverAction.takeSnapShot());
        } catch (Exception e) {
            GemTestReporter.addTestStep("Exception Occurred", "Exception: " + e, STATUS.FAIL);
            throw new RuntimeException(e);
        }
        ...
        try {
            ...
            if (unreadCount > 0) {
                ...
                if (unreadCount == notifList.size()) {
                    ...
                    GemTestReporter.addTestStep("Unread notifications", "Unread notifications count not matched, Expected: " + unreadCount + " Actual: " + notifList.size() + "", STATUS.FAIL, DriverAction.takeSnapShot());
    ...
    public void verifyReadNotificationButton() {
        try {
            ...
            if (!DriverAction.isExist(DashboardHeaderLocators.unreadCount) && DriverAction.getElementText(DashboardHeaderLocators.notificationListAlert).contains("No new")) {
                ...
                GemTestReporter.addTestStep("Notification read", "Notifications were not marked as read", STATUS.FAIL, DriverAction.takeSnapShot());
        ...
        try {
            ...
            if (DriverAction.isExist(DashboardHeaderLocators.notificationListContainer)) {
                ...
                if (DriverAction.getCurrentURL().contains(ProjectConfigData.getProperty("ticketDetailsUrl"))) {
                    ...
                    GemTestReporter.addTestStep("Ticket details", "Details page not displayed", STATUS.FAIL, DriverAction.takeSnapShot());
            }
        } catch (Exception e) {
            GemTestReporter.addTestStep("Exception Occurred", "Exception: " + e, STATUS.FAIL);
            throw new RuntimeException(e);
        ...
        try {
            if (!DriverAction.isExist(LoginLocators.employeeName)) {
                ...
                if (DriverAction.isExist(LoginLocators.employeeName)) {
                    ...
                    GemTestReporter.addTestStep("Toggle button", "Menu not expanded", STATUS.FAIL, DriverAction.takeSnapShot());

            } else
                GemTestReporter.addTestStep("Toggle button", "Side Menu not collapsed", STATUS.FAIL, DriverAction.takeSnapShot());
