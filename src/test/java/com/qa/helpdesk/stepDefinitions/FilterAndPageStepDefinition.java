...
public class FilterAndPageStepDefinition {
    ...
    public void verifyResultForSelection(String option) {
        try {
            ...
            if (elements.size() == Integer.parseInt(option)) {
                ...
                GemTestReporter.addTestStep("Pagination", "Rows per page displayed not as expected", STATUS.FAIL, DriverAction.takeSnapShot());
        } catch (Exception e) {
            GemTestReporter.addTestStep("Exception Occurred", "Exception: " + e, STATUS.FAIL);
            throw new RuntimeException(e);
        }
    ...
    public void openFilterOption() {
        try {
            if (DriverAction.isExist(FilterAndCalendarLocators.filterButton)) {
                ...
                GemTestReporter.addTestStep("Filter button", "Filter button not found", STATUS.FAIL, DriverAction.takeSnapShot());
        } catch (Exception e) {
            GemTestReporter.addTestStep("Exception Occurred", "Exception: " + e, STATUS.FAIL);
            throw new RuntimeException(e);
        }
    ...
    public void selectFilterCriteria(String dept, String status) {
        try {
            ...
            if (!status.isEmpty()) {
                ...
                GemTestReporter.addTestStep("Filter selection", "No filter selection is made", STATUS.INFO, DriverAction.takeSnapShot());
        } catch (Exception e) {
            GemTestReporter.addTestStep("Exception Occurred", "Exception: " + e, STATUS.FAIL);
            throw new RuntimeException(e);
        }
    ...
    public void verifyResultForSelection(String dept, String status) {
        try {
            if (DriverAction.isExist(FilterAndCalendarLocators.closeFilterButton)) {
            ...
            if (!dept.isEmpty()) {
                ...
                if (!DriverAction.isExist(TableAndPaginationLocators.noTableDataImg)) {
                    ...
                    if (deptFlag == 1) {
                        ...
                        GemTestReporter.addTestStep("Dept filter", "Dept filter results are not as expected", STATUS.FAIL, DriverAction.takeSnapShot());

                } else
                    GemTestReporter.addTestStep("Dept filter", "No results for filter dept " + dept + "", STATUS.INFO, DriverAction.takeSnapShot());
            }
            ...
            if (!status.isEmpty()) {
                ...
                if (!DriverAction.isExist(TableAndPaginationLocators.noTableDataImg)) {
                    ...
                    if (statusFlag == 1) {
                        ...
                        GemTestReporter.addTestStep("Status filter", "Status filter results are not as expected", STATUS.FAIL, DriverAction.takeSnapShot());

                } else
                    GemTestReporter.addTestStep("Status filter", "No results for filter status " + status + "", STATUS.INFO, DriverAction.takeSnapShot());
        ...
        try {
            ...
            if (DriverManager.getWebDriver().findElement(TableAndPaginationLocators.nextPageButton).isEnabled()) {
                ...
                if ((Integer.parseInt(option) + 1) == Integer.parseInt(displayedRows) && rows == Integer.parseInt(option)) {
                    ...
                    GemTestReporter.addTestStep("Next page button", "Some issue while navigating to next page", STATUS.PASS, DriverAction.takeSnapShot());

            } else
                GemTestReporter.addTestStep("Next page button", "Next page button is disabled", STATUS.INFO, DriverAction.takeSnapShot());
            ...
            if (DriverManager.getWebDriver().findElement(TableAndPaginationLocators.prevPageButton).isEnabled()) {
                ...
                GemTestReporter.addTestStep("Prev page button", "Prev page button is disabled", STATUS.INFO, DriverAction.takeSnapShot());
    ...
    public void verifyIfClosedCancelledAndResolvedTicketsAreHidden() {
        try {
            ...
            if (!DriverAction.isExist(TableAndPaginationLocators.noTableDataImg)) {
                ...
                GemTestReporter.addTestStep("Status filter", "No results for the filter", STATUS.INFO, DriverAction.takeSnapShot());
